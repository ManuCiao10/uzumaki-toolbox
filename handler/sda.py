...
def sda(tracking_number):
    ...
    response = session.get(
        'https://www.sda.it/wps/portal/Servizi_online/dettaglio-spedizione', params=params, headers=headers)

    if invalid_tracking not in response.text:
        print_task("[sda %s] error: %s" %
                   (tracking_number, "invalid or unavailable tracking number..."), RED)
        return

    if response.status_code == 200:
        print_task("[sda %s] successful got session..." %
        ...
        try:
            ...
            print_task("[sda %s] error: %s" %
                       (tracking_number, "error while parsing data..."), RED)
            return

        print_task("[sda %s] successful got data..." % tracking_number, GREEN)

        try:
            something = json_data.get("lettereVettura")[
                0].get("dettaglioSpedizioni")[0]
        except:
            print_task("[sda %s] error: %s" %
                       (tracking_number, "error while parsing data..."), RED)
            return
