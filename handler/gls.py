...
def gls(company, tracking_number, tkn, city, address, number_house, province, zip_code, name, surname, instructions):
    import requests
    ...
    response = session.get('https://gls-italy.com/index.php',
                           params=params, headers=headers)

    text = "Enter a new delivery address"

    if response.status_code == 200:
        print_task("[gls %s] successful got session..." %
        ...
        if text in response.text:
            ...
            headers = {
                ...
                'Pragma': 'no-cache',
                'Referer': 'https://gls-italy.com/index.php?option=com_gls&view=svincola&mode=flexdelivery&firstspirit=5&loc=' + version + '&num=' + number + '&tkn=clljjkda',
                'Sec-Fetch-Dest': 'document',
                'Sec-Fetch-Mode': 'navigate',
            ...
            response = session.post('https://gls-italy.com/index.php?option=com_gls&amp;lang=it',
                                    cookies=cookies, headers=headers, data=data)

            if "Choose a new address:" in response.text:
                print_task("[gls %s] filling new address..." %
                ...
                data = "com_users&task=registration.getIndirizzo&format=raw&indirizzo=" + via + "+" + road + "&cap=" + zip_code + \
                    "&localita=" + city + "&provincia=" + province + \
                    "&sede_principale=-T1&comune=" + city + "&zona=**&flex=si"
                ...
                response = session.post('https://gls-italy.com/index.php',
                                        cookies=cookies, headers=headers, data=data)
                ...
                response = requests.post(
                    'https://gls-italy.com/index.php?option=com_gls&amp;lang=it', cookies=cookies, headers=headers, data=data)
        ...
        print_task("[gls %s] error: %s" %
                   (tracking_number, response.status_code), RED)
        return
